...
author: "Swiss Actuarial Association / Modified by Mahadev Suresh"
date: "`r Sys.Date()`"
output:
  html_document:
    df_print: paged
...
```{r}
setwd("~/Actuarial Data Science/Case 1 - French TPL")
...
area <- read_sf(dsn = "~/Actuarial Data Science/Case 1 - French TPL/shapefile/FRA_adm2.shp", layer = "FRA_adm2")
qtm(shp = area, fill = "ID_1", fill.palette = "-Blues", text = "ID_1") 
...
area_points <- area_points[order(area_points$ID_1), ]  # Has to be ordered correctly to plot.

area_points <- area_points %>% 
  select(ID_1, NAME_1, NAME_2, geometry) %>% 
  rename(id = ID_1, Region = NAME_1, Region_1 = ) %>%
  mutate(Region = case_when(Region == "Rhône-Alpes" ~ "Rhone-Alpes",
                            Region == "Franche-Comté" ~ "Franche-Comte", 
                            Region == "Île-de-France" ~ "Ile-de-France", 
                            Region == "Pays de la Loire" ~ "Pays-de-la-Loire", 
                            Region == "Midi-Pyrénées" ~ "Midi-Pyrenees",
                            Region == "Provence-Alpes-Côte d'Azur" ~ "Provence-Alpes-Cotes-D'Azur",
                            TRUE ~ Region))
```
...
area_points <- merge(
  area_points,
  reg_sum[, c("Exp","Freq","Area","VehPower","VehAge","DrivAge","BonusMalus","VehGas","Density", "Region")],
  by.x = "Region",
  by.y = "Region",
  all.x = TRUE
)
...
```{r}
qtm(shp = area_points, fill = "Exp", fill.palette = "-Blues")
```

```{r}
qtm(shp = area_points, fill = "Freq", fill.palette = "-Blues")
```


```{r}
qtm(shp = area_points, fill = "VehGas", fill.palette = "-Blues")
```

```{r}
qtm(shp = area_points, fill = "VehAge", fill.palette = "-Blues")
```
